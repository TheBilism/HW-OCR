 ...
 * Założenia
 *      
 *      -   Wymiar odczytywanego obrazka 28x28 -> 784 px wejściowe
 *      -   Alfabet bez polskich znaków 26 liter
 *      -   
 */


namespace OCR.NEURONNETWORK
{
    public class neuron
    {
        double[] weights;
    ...
    public class network
    {
        ...
        public network(int numLayers, int numInputs, byte[] npl)
        {
...
/*
namespace testocr.SiecNeuronowa
{
    public class Vector
    {
        public int size;
    ...
    {
        public int inpNodeSize;
        public int inpLayerSize;
        public int hidNodeSize;
        public int hidLayerSize;
        public int outNodeSize;
    ...
    public class siecneuronowa
    {
        ...
        //LayerType;
        public enum ActFctType { SIGMOID, TANH };

        Network CreateNetwork(int inpCount, int hidCount, int outCount)
        {
            int inpNodeSize = Marshal.SizeOf(typeof(Node));         // Input layer has 0 weights
            int inpLayerSize = Marshal.SizeOf(typeof(Layer)) + (inpCount * inpNodeSize);

            // Calculate size of HIDDEN Layer
            int hidWeightsCount = inpCount;
            int hidNodeSize = Marshal.SizeOf(typeof(Node)) + (hidWeightsCount * sizeof(double));
            int hidLayerSize = Marshal.SizeOf(typeof(Layer)) + (hidCount * hidNodeSize);

            // Calculate size of OUTPUT Layer
            int outWeightsCount = hidCount;
            int outNodeSize = Marshal.SizeOf(typeof(Node)) + (outWeightsCount * sizeof(double));
            int outLayerSize = Marshal.SizeOf(typeof(Layer)) + (outCount * outNodeSize);
            ...
            //List<Network> nn = new List<Network>(); //(Network)malloc(sizeof(Network) + inpLayerSize + hidLayerSize + outLayerSize);

            // Allocate memory block for the network
            Network nn = new Network(); //(Network*)malloc(sizeof(Network) + inpLayerSize + hidLayerSize + outLayerSize);
            ...
            initWeights(nn, LayerType.HIDDEN);
            initWeights(nn, LayerType.OUTPUT);
        ...
        void initNetwork(Network nn, int inpCount, int hidCount, int outCount)
        {
            ...
            List<Layer> il = createInputLayer(inpCount);
        ...
        List<Layer> createInputLayer(int inpCount)
        {
